...
contract Land{
    ...
    function getStatus(uint256 _id)public view returns(uint256){
        if(plots[_id].status==LandLibrary.Status.notForSale)return 1;
        if(plots[_id].status==LandLibrary.Status.forSale)return 2;
        if(plots[_id].status==LandLibrary.Status.forBid)return 3;
        return 0;
    }
